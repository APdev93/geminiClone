...
function hidePreview() {
	let prevCtr = document.getElementById("imgPrev");
	let imageInput = document.getElementById("imageInput");
	prevCtr.style.display = "none";
...
async function showPreview(event) {
	if (event.target.files.length > 0) {
		...
		let prevCtr = document.getElementById("imgPrev");
		prevCtr.style.display = "flex";

		// Tampilkan preview gambar
		let imagePreview = document.getElementById("imagePrev");
		let fileURL = URL.createObjectURL(src); // Konversi file ke URL
		imagePreview.src = fileURL;
		imagePreview.style.display = "block";

		// 		// Upload gambar
		// 		let data = await uploadImages(src);
		// 		if (data) {
...
const requestApiResponse = async (incomingMessageElement) => {
	...
	if (imageInput.value) {
		if (imageInput.files[0]) {
			...
			hidePreview();
			let uploadedImg = await uploadImages(imageInput.files[0]);
			if (uploadImages) {
				messages = {
